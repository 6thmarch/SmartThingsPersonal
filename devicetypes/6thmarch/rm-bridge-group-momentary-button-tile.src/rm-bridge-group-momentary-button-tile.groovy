 ...
 *	
 *	RM Bridge Group Momentary Button Tile 
 *	Version : 1.0.1
 * 
 * 	Description:
 * 		RM Bridge Group Momentary Button Tile is a SmartThings Device Type that allows you to turn on or off devices 
 ...
 *
 *  2016-02-11  V1.0.0  Initial release
 *	2016-02-29 	V1.0.1	Device type renamed
 */
  import groovy.transform.Field
 @Field final int MAX_CODES_PER_GROUP = 10 //on() and off() need to be changed if this value is edited.
 
metadata {
	definition (name: "RM Bridge Group Momentary Button Tile", 
	 ...
	 preferences {
       ...
       input "passwd", "password", title: "Password",
		...
		1.upto(MAX_CODES_PER_GROUP, {
       		 input "code${it}", "text", title: "Code ${it}",
              description: "This is the code to send to Android RM Bridge if the button is pressed. e.g. FanOn",
              required: false, displayDuringSetup: true
...
def push() {
    ...
    if("${code1}" != "null"){
         makeJSONBroadlinkRMBridgeRequest("${code1}")
    }
    if("${code2}" != "null"){
         makeJSONBroadlinkRMBridgeRequest("${code2}")
    }
    if("${code3}" != "null"){
         makeJSONBroadlinkRMBridgeRequest("${code3}")
    }
    if("${code4}" != "null"){
         makeJSONBroadlinkRMBridgeRequest("${code4}")
    }
    if("${code5}" != "null"){
         makeJSONBroadlinkRMBridgeRequest("${code5}")
    }
    if("${code6}" != "null"){
         makeJSONBroadlinkRMBridgeRequest("${code6}")
    }
    if("${code7}" != "null"){
         makeJSONBroadlinkRMBridgeRequest("${code7}")
    }
    if("${code8}" != "null"){
         makeJSONBroadlinkRMBridgeRequest("${code8}")
    }
    if("${code9}" != "null"){
         makeJSONBroadlinkRMBridgeRequest("${code9}")
    }
    if("${code10}" != "null"){
         makeJSONBroadlinkRMBridgeRequest("${code10}")
    }
